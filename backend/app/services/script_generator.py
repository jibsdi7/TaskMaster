"""
Playwright / PyAutoGUI Script Generator
Generates web scripts (Playwright) and desktop/hybrid scripts (PyAutoGUI).
"""
import re
...
class ScriptGenerator:
    """Generate Playwright / PyAutoGUI scripts from workflow nodes"""
    ...
    def generate(
        ...
        For "desktop" and "hybrid" languages, the PyAutoGUI generator is used.
        For "python" / "javascript" / "typescript" the Playwright generator is used.
        If the workflow contains only desktop nodes, "python" language automatically
        falls through to the desktop generator.
        """
        ordered_nodes = topological_sort(nodes, edges)

        has_desktop = any(n.get("node_type", "") in _DESKTOP_NODE_TYPES for n in ordered_nodes)
        has_web = any(n.get("node_type", "") in _WEB_NODE_TYPES for n in ordered_nodes)

        # Auto-detect mode when language is not explicitly desktop/hybrid
        if language == "desktop" or (has_desktop and not has_web):
            return DesktopScriptGenerator.generate(
                ordered_nodes, include_comments=include_comments, mode="desktop"
            )
        if language == "hybrid" or (has_desktop and has_web):
            return DesktopScriptGenerator.generate(
                ordered_nodes, include_comments=include_comments, mode="hybrid"
            )
    ...
    def _generate_python(
        ...
        # Header
        if include_comments:
            lines.append("# Generated by IBMTaskWeaver")
